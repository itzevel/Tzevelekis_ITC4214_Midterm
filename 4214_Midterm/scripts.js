$(document).ready(function () {
    ...
    $("#filterPriority").change(function () {
      const val = $(this).val();
      if (val === "All") renderTasks();
      else {
        const filtered = tasks.filter(t => t.priority === val);
        $("#taskList").empty();
        filtered.forEach((task, index) => {
          $("#taskList").append(`<tr><td>${task.name}</td><td>${task.description}</td><td>${task.due}</td><td>${task.priority}</td><td>${task.status}</td><td>Filtered</td></tr>`);
        });
      }
    });
  });
